            ...
            for (const transaction_receipt &receipt : next->transactions)
            {
                  if (receipt.trx.contains<packed_transaction>())
                  {
                        ...
                        if (filter)
                        {
                              ...
                              *out << fc::json::to_pretty_string(unpacked_transaction_variant) << "\n";
                        }
                  }
            }
...
{
      cli.add_options()("blocks-dir", bpo::value<bfs::path>()->default_value("blocks"), "the location of the blocks directory (absolute path or relative to the current directory)")
      ("output-file,o", bpo::value<bfs::path>(), "the file to write the block log output to (absolute or relative path).  If not specified then output is to stdout.")
      ("eosio-abi-path", bpo::value<std::string>(), "eosio-abi-path")
      ("first", bpo::value<uint32_t>(&first_block)->default_value(1), "the first block number to log")
      ("last", bpo::value<uint32_t>(&last_block)->default_value(std::numeric_limits<uint32_t>::max()), "the last block number (inclusive) to log")
      ("no-pretty-print", bpo::bool_switch(&no_pretty_print)->default_value(false), "Do not pretty print the output.  Useful if piping to jq to improve performance.")
      ("as-json-array", bpo::bool_switch(&as_json_array)->default_value(false), "Print out json blocks wrapped in json array (otherwise the output is free-standing json objects).")
...
void blocklog::initialize(const variables_map &options)
{
      try
      {
            ...
            eosio_token_abi_def = fc::json::from_file(options.at("eosio-abi-path").as<std::string>()).as<abi_def>();
      }
      FC_LOG_AND_RETHROW()
}
